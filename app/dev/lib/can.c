...
//    can_mode_set(1, MODE_NORM);             //Set Loopback mode
    ...
    //Timing configuration Brate = 250 Kbits/s
    C1CFGbits.BRP = 7;                      // Tq = (2 X (BRP + 1))/ Fsys
    C1CFGbits.PRSEG = 2;                    // 3 Tq in propagation segment
    C1CFGbits.SEG1PH = 2;                   // 3 Tq in phase 1 (and 2 auto)
    C1CFGbits.SEG2PH = 2;                   // 3 Tq in phase 1 (and 2 auto)
    C1CFGbits.SEG2PHTS = 1;                   // 3 Tq in phase 1 (and 2 auto)
    C1CFGbits.SJW = 0;
    C1CFGbits.SAM = 1;// (SJW+1)*Tq adjustment window
    can_mode_set(1, MODE_NORM);             //Set Loopback mode
...
void can2_init(){
    ...
    C2CFGbits.SEG2PH = 2;                   // 3 Tq in phase 1 (and 2 auto)
    C2CFGbits.SEG2PHTS = 1;                   // 3 Tq in phase 1 (and 2 auto)
    C2CFGbits.SJW = 0;  
    C2CFGbits.SAM = 1;// (SJW+1)*Tq adjustment window
